...
Yep, I started using Mac OS, and have therefore updated the scripts accordingly. The scripts default to Linux file paths, but will detect a Darwin platform (using `uname -s`) and adjust itself accordingly. As of this writing, you can use a dedicated symlink for Darwin platforms by appending `.darwin` to the symlink file (ie - `nanorc.symlink.darwin`). This will use the .darwin symlink instead of the plain .symlink file.

For now, most of the OS detection stuff is in the symlink files themselves (save for nanorc, which doesn't support logic, hence the dedicated files), but I'm quickly finding OSX to be quite needy of the .bashrc/.bash_profile, so I might make it a dedicated file in the (near) future.
